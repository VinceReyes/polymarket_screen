...
import { createRenderJob, startRenderJob, type RenderJobResult } from '@/lib/render-jobs';
...
async function renderFromBody(body: any): Promise<RenderJobResult> {
  ...
  if (Array.isArray(body?.options) && body.options.length >= 2) {
    ...
    if (requests.length < 2) {
      throw new Error('Compare mode needs two valid option ids');
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    startRenderJob(job.id, () => renderFromBody(body));
